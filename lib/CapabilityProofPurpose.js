...
module.exports = class CapabilityProofPurpose extends ControllerProofPurpose {
  // FIXME: need to figure out common params for subclasses and update docs
  // and constructor
  ...
  async validate(proof, {verificationMethod, documentLoader, expansionMap}) {
    ...
    try {
      ...
      const validateResult = await this._runChecksAfterChainVerification({
        dereferencedChain, documentLoader, expansionMap, proof,
        verificationMethod, capabilityChainMeta
      });
  ...
  async _runSuperProofValidation({
