...
module.exports = class CapabilityInvocation extends ControllerProofPurpose {
  ...
  constructor({
    ...
    capability, capabilityAction, invocationTarget, date,
    ...
    if(capability !== undefined || capabilityAction !== undefined ||
      invocationTarget !== undefined || date !== undefined) {
      // purpose to be used to create a proof, so these should not be specified
      if(controller || currentDate ||
        expectedAction || expectedRootCapability || expectedTarget ||
        inspectCapabilityChain || suite) {
        throw new Error(
          'If parameters for creating a proof are specified, parameters ' +
          'for verifying one must not be present.');
      }
      ...
      // purpose to be used to verify a proof, so these should not be specified
      if(capability || capabilityAction || invocationTarget || date) {
        throw new Error(
          'If parameters for verifying a proof are specified, parameters ' +
          'for creating one must not be present.');
      }
  ...
  async validate(proof, {verificationMethod, documentLoader, expansionMap}) {
    ...
    try {
      ...
      // FIXME: require `capability` to be string if it is root and to be
      // object if is delegated
