...
api.getDelegator = capability => {
  ...
  return delegator || controller || id;
};
  ...
  */
  ...
  try {
    ...
    if(allowedAction.length > 0) {
      if(!capabilityAction) {
        throw new Error(
          'No "capabilityAction" specified when required; ' +
          'allowed actions are: ' +
          allowedAction.map(x => `"${x}"`).join(', '));
      }
      if(!allowedAction.includes(capabilityAction)) {
        throw new Error(
    ...
    if(isRoot) {
      // ensure that the invocation target matches the root capability
      if(api.getTarget(root) !== root.id) {
        throw new Error(
