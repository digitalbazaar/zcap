...
// FIXME: remove this; only fetch root zcaps and potentially do it via
// a separate hook? ... and validate what is returned as having a rigid
// required structure instead of doing JSON-LD transforms
...
api.fetchInSecurityContext = async ({
  ...
  // FIXME: remove this entire helper function
  console.log('should never occur', zcap);
  process.exit(1);
};
...
api.validateCapabilityChain = ({
  capability, capabilityChain, maxChainLength = MAX_CHAIN_LENGTH
}) => {
 ...
 *
 * @param {object} capability - The authorization capability (zcap) to get
 *   the chain for.
 ...
 *   chain (this is inclusive of `capability` itself).
 *
 * @return {Promise<object>} {dereferencedChainMap, dereferencedChain}.
 */
api.dereferenceCapabilityChain = async ({
  ...
  // dereference root zcap
  // FIXME: add note that root zcap must always be dereferenced since
  // it does not have a delegation proof and the verifier must provide a
  // trusted version of it -- this code may need to be tweaked in the
  // invocation case or it may need to be the place where the root zcap
  // is retrieved in the invocation case (instead of it happening externally)
  // ... which would mean that `capability` could be a string -- but only for
  // the root zcap case
  if(capabilityChain.length > 0) {
    const [id] = capabilityChain;
    const {rootCapability} = await getRootCapability({id});
    _validateCapability({capability: rootCapability});
  ...
  // FIXME: remove `dereferencedChainMap` if unnecessary
  return {dereferencedChainMap, dereferencedChain};
};
...
api.verifyCapabilityChain = async ({
  ...
  /*  Note: We start verifying a capability from its root of trust (the
    ...
    5. For each capability `cap` in the chain, verify the capability delegation
  ...
  */
  ...
  try {
    ...
    // FIXME: move `validateCapabilityChain` outside of this call and make
    // it a requirement to call this function with a validated chain? that
    // would eliminate weirdness with checking from root => tail... so new
    // process would be:
    // 1. get the whole chain -- which is safe from attack now because only
    //    the root is resolved from an external source, all delegated zcaps
    //    MUST be present
    // 2. then verify the whole chain
    // FIXME: ... this would also give better clarity around the process and
    // what the invariants are at various stages
    // FIXME: make a `dereferenceCapabilityChain` function to replace
    // `validateCapabilityChain` that builds the fully de-ref'd chain,
    // including de-ref'ing the root zcap, and it can throw directly
    ...
    // let root = isRoot ? capability : capabilityChain[0];
    ...
    const dereferencedChainMap = new Map();
    for(const zcap of dereferencedChain) {
      dereferencedChainMap.set(zcap.id, zcap);
    }
    ...
    // 5. For each capability `cap` in the chain, verify the capability
    ...
    // get full dereferenced chain *without* root zcap
    const dereferencedChain2 = dereferencedChain.slice(1);
    const capabilityChainMeta = [];
    const lastIndex = dereferencedChain2.length - 1;
    for(let i = 0; i < dereferencedChain2.length; ++i) {
      const cap = dereferencedChain2[i];
      if(typeof cap !== 'object') {
        // FIXME: does this need to be checked or are we guaranteed to have
        // all deref'd and valid zcaps now?
      }
      let verifyResult;
    ...
    for(const c of dereferencedChain2) {
    ...
    if(inspectCapabilityChain) {
      const {valid, error} = await inspectCapabilityChain({
        ...
        capabilityChain: dereferencedChain2,
        capabilityChainMeta,
      });
      if(!valid) {
        throw error;
