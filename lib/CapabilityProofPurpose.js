...
module.exports = class CapabilityProofPurpose extends ControllerProofPurpose {
   ...
   *   that the date on the tail capability delegation or the capability
   *   invocation proof (depends on the derived class type) can deviate from
  ...
  async validate(proof, validateOptions) {
    ...
    try {
      ...
      const {
        ...
        maxChainLength,
        maxDelegationTtl,
        // intentionally do not pass `maxTimestampDelta` as it only applies
        // to the tail proof or the invocation proof and that proof's created
        // date will be checked for monotonicity against the rest of the chain
        suite
      });
      if(!verified) {
        throw error;
