...
api.verifyCapabilityChain = async ({
  ...
  */
  ...
  try {
    ...
    // FIXME: just use dereferencedCapabilities var
    const dereferencedChain = [...dereferencedChainMap.values()];
    // skip root zcap (first in chain, start at i = 1)
    // eslint-disable-next-line prefer-const
    for(let i = 0; i < capabilityChain.length; ++i) {
    //for(let [i, cap] of capabilityChain.entries()) {
      // skip root zcap since its was sliced from `capabilityChain`
      const cap = dereferencedChain[i + 1];
