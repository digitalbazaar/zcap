...
// FIXME: determine how to combine `isValidTarget` with local helper in
// `CapabilityInvocation`

/**
 * Determines if the given `invocationTarget` is valid given the
 * `parentInvocationTarget`.
 *
 * @param {string} invocationTarget - The invocation target to check.
 * @param {string} parentInvocationTarget - The parent invocation target.
 ...
 */
api.isValidTarget = ({
  invocationTarget, parentInvocationTarget, allowTargetAttenuation
}) => {
  // direct match, valid
  if(parentInvocationTarget === invocationTarget) {
    return true;
  }
  if(allowTargetAttenuation) {
    ...
    const prefix = `${parentInvocationTarget}/`;
 ...
 * @param [expectedRootCapability] {string|array} - The expected root
...
api.verifyCapabilityChain = async ({
  dereferencedChain, expectedRootCapability,
  proofVerifyResult, validateAgainstParent,
  ...
  2. For each capability `cap` in the chain, verify the capability delegation
  ...
  try {
    ...
    // next, ensure the root capability is as expected
    ...
    // ... in which case we may need an option to skip this check?
    ...
    // 3. For each capability `zcap` in the chain, verify the capability
    ...
    for(let i = 0; i < delegatedCapabilities.length; ++i) {
      ...
      if(i === lastIndex && proofVerifyResult) {
        ...
        verifyResult = await jsigs.verify(zcap, {
          suite,
          purpose: new CapabilityDelegation({
            ...
            // FIXME: add missing params for maxZZZ, etc. ?
          }),
          documentLoader,
          expansionMap
        });
    ...
    for(const c of delegatedCapabilities) {
      ...
      // FIXME: ensure these rules are proper
      const invocationTarget = api.getTarget({capability: c});
