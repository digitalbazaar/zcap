 ...
 * that every zcap starting after the root of the chain has been properly
 * delegated.
 *
...
api.verifyCapabilityChain = async ({
  dereferencedChain, proofVerifyResult, validateAgainstParent,
  ...
  /*  Note: We start verifying a capability from its root of trust (the
    ...
    1. Verify the root capability:
      1.1. Check the expected target, if one was specified.
      1.2. Ensure root capability is expected and has an invocation target.
  ...
  */
  ...
  // not required for delegation checks (therefore, this check should only be
  // performed on the invocation itself not in the chain verification -- there
  // are other FIXMEs about this)
  ...
  try {
    ...
    // FIXME: determine if `expectedRootCapability` is required for delegation
    // proof only checks...
