...
api.verifyCapabilityChain = async ({
  capability, proofVerifyResult, validateAgainstParent,
  ...
  */
  ...
  try {
    ...
    const dereferencedChain = [...dereferencedChainMap.values()].slice(1);
    const capabilityChainMeta = [];
    const lastIndex = dereferencedChain.length - 1;
    // FIXME: can we use `dereferencedChain` here?
    for(let i = 0; i < dereferencedChain.length; ++i) {
      const cap = dereferencedChain[i];
    ...
    for(const c of dereferencedChain) {
