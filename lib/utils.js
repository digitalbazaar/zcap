...
const {
  // FIXME: remove `SECURITY_CONTEXT_V2_URL` once no longer used
  MAX_CHAIN_LENGTH, SECURITY_CONTEXT_V2_URL, ZCAP_CONTEXT_URL, ZCAP_ROOT_PREFIX
} = require('./constants');
...
api.fetchInSecurityContext = async ({
  ...
  if(ctx === SECURITY_CONTEXT_V2_URL ||
    ctx === ZCAP_CONTEXT_URL ||
    (Array.isArray(ctx) && ctx[0] === ZCAP_CONTEXT_URL)) {
    return zcap;
  }

  return jsonld.compact(zcap, SECURITY_CONTEXT_V2_URL, {
