...
api.verifyCapabilityChain = async ({
  ...
  2. For each capability `cap` in the chain, verify the capability delegation
    proof on `cap` (using `partialVerifyResult` for `capability` if it was
  ...
  try {
    ...
    for(let i = 0; i < delegatedCapabilities.length; ++i) {
      ...
      let verifyResult;
      if(i === lastIndex && partialVerifyResult) {
        /*
        ...
        verifyResult = {
          ...
          results: [{...partialVerifyResult, purposeResult}]
        };
      ...
      // FIXME: simplify conditional or move this one line up
      if(!(i === lastIndex && partialVerifyResult)) {
        // zcap verified; save meta data for later inspection
