  ...
  */
  ...
  try {
    ...
    for(const c of fullCapabilityChain) {
      ...
      const isRoot = !!parent;
      const getDelegatedDate = !isRoot &&
        (requireChainDateMonotonicity || maxDelegationTtl < Infinity);
      if(getDelegatedDate) {
      ...
      if(!isRoot && requireChainDateMonotonicity) {
      ...
      if(!isRoot && maxDelegationTtl < Infinity) {
        ...
        2. The zcap's delegation date is not in the future.
        3. The zcap's expiration date is not before its delegation date.
        4. The zcap's current TTL is <= `maxDelegationTtl`
        5. The zcap's TTL was never > `maxDelegationTtl`. */
        if(c.expires === undefined) {
          throw new Error(
        ...
        if(currentCapabilityDelegationDate > currentCapabilityExpirationDate) {
          throw new Error(
            'A delegated capability in the delegation chain expires ' +
            'before it was delegated.');
        }
