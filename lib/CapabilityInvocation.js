  ...
  async validate(proof, {verificationMethod, documentLoader, expansionMap}) {
    ...
    try {
      ...
      const purposeParameters = {
        expectedTarget, expectedRootCapability, CapabilityDelegation, suite
      };
      ...
      // invoked zcap's chain has a reference to a root zcap
      // the verifier must provide a hook to deref this root zcap; if it
      //   can't be deref'd by the trusted hook it's a not authz error,
      //   otherwise...
      // root zcap ID is compared against a list of expected root zcaps, if
      //   it doesn't match any of them, throw not authz error, otherwise...
      // then zcap chain verification ensures that the combination of
      //   a particular target + a root zcap is permitted
      //   and it's conceivable that more than one combination is
      //   allowed on a given system: a target of `x` could work
      //   with both root zcap `a` and `b`
...
// FIXME: determine how to consolidate with checks performed during chain
// verification
