  ...
  try {
    ...
    for(const c of fullCapabilityChain) {
      ...
      if(requireChainDateMonotonicity) {
        // verify parent capability was created before child
        // FIXME: implement
      }

      if(maxDelegationTtl < Infinity) {
        // FIXME: ensure all non-root zcaps have `expires` and that
        // `expires - delegationProof.created` is <= `maxDelegationTtl`
        // FIXME: using `delegationProof.created` won't work unless the
        // system doesn't allow created times in the future, which perhaps
        // it should do as well ... alternatively, it could just just use
        // `now` (overridable) to compare against `expires`
      }
