  ...
  async _verifyCapabilityChain({
    ...
    try {
      ...
      for(let i = 0; i < delegatedCapabilities.length; ++i) {
        ...
        let currentCapabilityDelegationTime;
        ...
        for(const p of proofs) {
          ...
          if(currentCapabilityDelegationTime === undefined) {
            currentCapabilityDelegationTime = created;
          } else if(currentCapabilityDelegationTime < created) {
            /* Note: Intentionally do not use `utils.compareTime` as there is
            no clock drift issue here. We are not comparing against any live
            values but against date-time values expressed in the chain. We
            want the earliest time and we don't want to erroneously choose
            a later time because the times are equivalent within a range. */
            currentCapabilityDelegationTime = created;
          }
        }
