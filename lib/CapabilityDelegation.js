...
module.exports = class CapabilityDelegation extends ControllerProofPurpose {
  ...
  async validate(
    ...
    try {
      // FIXME: run utils.validateCapability() instead and add a flag to
      // expect it to be delegated (and not root)
      ...
      // FIXME: eliminate duplicate calls to this
  ...
  async update(proof, {document, documentLoader, expansionMap}) {
    ...
    if(!capabilityChain) {
      ...
      capabilityChain = await utils.computeCapabilityChain(
        {capability: document, documentLoader, expansionMap});
    }
