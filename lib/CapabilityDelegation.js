...
module.exports = class CapabilityDelegation extends ControllerProofPurpose {
  ...
  constructor({
    ...
    // FIXME: remove `expectedRootCapability` as it is only relevant to
    // invocation checks?
  ...
  async validate(
    ...
    try {
      ...
      const purposeParameters = {
        expectedRootCapability, CapabilityDelegation, suite
      };
      ...
      async function validateAgainstParent() {
        ...
        // FIXME: move this check into `verifyCapabilityChain`
