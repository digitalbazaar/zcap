...
module.exports = class CapabilityDelegation extends ControllerProofPurpose {
  ...
  constructor({
    ...
    capability, capabilityChain, date, parentCapability,
    // proof verification params
    allowTargetAttenuation = false,
    capabilityAction, controller, currentDate,
    expectedRootCapability, expectedTarget, inspectCapabilityChain,
    ...
    // FIXME: why is `capabilityAction` present?
    const hasCreateProofParams = capability || capabilityChain || date ||
      parentCapability;
    if(hasCreateProofParams) {
      // purpose to be used to create a proof, so these should not be specified
      if(capabilityAction || controller || currentDate ||
        expectedRootCapability || expectedTarget ||
