...
export function hasValidAllowedAction({allowedAction, parentAllowedAction}) {
  ...
  /* Note: `allowedAction` can be expressed as either a string or a non-empty
  array (see `checkCapability`), so compare the action sets, not the JSON
  types used to express them. */
  const parentActions = getAllowedActions(
    {capability: {allowedAction: parentAllowedAction}});
...
export function checkCapability({capability, expectRoot}) {
  ...
  if(isRoot) {
    ...
    /* Note: The first entry in a delegated capability's chain is the ID of
    the root capability, which is what anchors the chain to a root zcap that
    the verifier dereferences via a trusted mechanism. An empty chain cannot
    be anchored to any root and MUST be rejected here; only a root capability
    has an empty chain. */
